 ...
 *
 * DESTRUCTIVE: this deletes every slot, click, reservation and Wall entry. It is meant
 * for a scratch database, and it refuses to run against one holding completed orders,
...
import { getUpcomingSlots } from "../lib/slots";
 ...
 *
 * This script has destroyed a production board once already. Both tables are checked,
 * because a Wall entry outlives its slot: looking at slots alone would let a database
 * full of real Wall purchases sail straight past the refusal.
 */
...
async function main() {
  ...
  await check("there is no stored price left to reconcile", async () => {
    const rows = await query<{ n: string }>(
      `SELECT count(*)::text AS n FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'board'`,
    );
    assert.equal(rows[0].n, "0", "the board table must be gone after migrate");
  });
  ...
  await check("an expired reservation returns the slot to open", async () => {
    ...
    await query(
      `INSERT INTO reservations (slot_id, amount, expires_at, status)
       VALUES ($1, 500, now() - interval '1 minute', 'pending')`,
      [slotId],
    );
  ...
  await check("a live reservation on a future hour survives reconcile", async () => {
    // Reconcile runs on every page render, so an over-eager expiry rule would kill a
    ...
    await query(`UPDATE slots SET status = 'reserved' WHERE id = $1`, [slot[0].id]);
    await query(
      `INSERT INTO reservations (slot_id, amount, expires_at, status)
       VALUES ($1, 500, now() + interval '9 minutes', 'pending')`,
      [slot[0].id],
    );
    ...
    assert.equal(res[0].status, "pending", "a live reservation must not be expired");
    const after = await query<{ status: string }>(
      `SELECT status FROM slots WHERE id = $1`,
      [slot[0].id],
    );
    assert.equal(after[0].status, "reserved", "slot must stay held for the buyer");
  });

  await check("a reservation on a finished hour is expired", async () => {
    await query(`DELETE FROM reservations`);
    await query(
  ...
  await check("hours are stored on exact UTC hour boundaries", async () => {
    // lib/db.ts pins the session to UTC. On a :30-offset session zone date_trunc would
    // floor to a :30 boundary and every `starts_at = date_trunc(...)` lookup would miss.
    const rows = await query<{ tz: string; bad: string }>(
      `SELECT current_setting('TimeZone') AS tz,
              (SELECT count(*)::text FROM slots
                WHERE EXTRACT(minute FROM starts_at) <> 0
                   OR EXTRACT(second FROM starts_at) <> 0) AS bad`,
    );
    assert.equal(rows[0].tz, "UTC", "the pg session must be pinned to UTC");
    assert.equal(rows[0].bad, "0", "every slot must start exactly on the hour");
  });
