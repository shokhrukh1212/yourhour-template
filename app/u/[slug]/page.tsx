    ...
    <main className="flex-1 px-6 py-16">
      <div className="mx-auto w-full max-w-xl">
        ...
        <section className="mt-10 rounded-[20px] border border-border bg-surface p-6">
          <CampaignClickProgress campaignId={campaign.id} accountingStatus={campaign.accounting_status} purchasedClicks={campaign.purchased_clicks} initialGuaranteed={campaign.guaranteed_clicks_delivered} initialTotal={campaign.total_clicks_delivered} initialBonusClicks={campaign.bonus_clicks_delivered} />
        ...
        <div className="mt-12 border-t border-border pt-6"><p className="text-sm text-faint">This product&apos;s listing, link and leaderboard rank stay visible permanently.</p><div className="mt-4 flex flex-wrap gap-3"><Link href="/get-clicks" className="inline-flex min-h-11 items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-medium hover:border-accent hover:text-accent">Get your own clicks <span aria-hidden="true">→</span></Link><Link href="/#wall" className="inline-flex min-h-11 items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-medium hover:border-accent hover:text-accent">See the leaderboard</Link></div></div>
      </div>
    </main>
  );
}
