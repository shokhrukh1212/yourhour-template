...
export function Wall({
  ...
  const { clicks, bonusClicks, refresh } = useClicks();
  const timeouts = useRef<ReturnType<typeof setTimeout>[]>([]);
  const onTrackedClick = useCallback(() => AFTER_CLICK_MS.forEach((delay) => timeouts.current.push(setTimeout(refresh, delay))), [refresh]);
  useEffect(() => {
  ...
  return (
    <section id="wall" className="landing-shell scroll-mt-40 pb-36">
      ...
      <div className="grid items-start gap-5 lg:grid-cols-[3fr_2fr]">
        <div>
          {entries.length ? <div className="grid gap-3.5">{entries.map((entry, index) => <WallCard key={entry.id} entry={entry} rank={firstRank + index} featured={page === 1 && index === 0} clicks={clicks[entry.id] ?? entry.clicks_delivered} bonusClicks={bonusClicks[entry.id] ?? entry.bonus_clicks} onTrackedClick={onTrackedClick} />)}</div> : <div className="rounded-[22px] border border-border bg-surface p-8 text-center text-muted"><p className="text-lg font-semibold text-foreground">The leaderboard is ready for #1.</p><p className="mt-2">The first product stays listed permanently.</p></div>}
          {totalPages > 1 ? <nav className="mt-6 flex items-center justify-between text-sm">{page > 1 ? <Link href={`/?wall=${page - 1}#wall`} className="font-semibold text-accent hover:underline">← Higher up</Link> : <span />}<span className="text-faint tabular">Page {page} of {totalPages} · {total.toLocaleString()} products</span>{page < totalPages ? <Link href={`/?wall=${page + 1}#wall`} className="font-semibold text-accent hover:underline">Further down →</Link> : <span />}</nav> : null}
        </div>
...
function WallCard({ entry, rank, featured, clicks, bonusClicks, onTrackedClick }: { entry: PublicCampaign; rank: number; featured: boolean; clicks: number; bonusClicks: number; onTrackedClick: () => void }) {
  const paidClicks = paidClicksForDisplay(entry);
  return (
    <article className={`flex min-w-0 flex-col rounded-[22px] border bg-surface transition hover:-translate-y-0.5 hover:border-white/20 ${featured ? "min-h-[250px] border-accent/40 bg-[linear-gradient(90deg,rgba(215,255,103,.07),transparent_58%),#101219] p-8" : "min-h-[220px] border-border p-6"}`}>
      ...
      <div className="mt-auto flex flex-wrap items-end gap-4 pt-7 text-xs text-faint"><a href={`/r/${entry.id}`} target="_blank" rel="noopener" onClick={onTrackedClick} className="mb-0.5 inline-flex items-center gap-2 font-bold text-foreground hover:text-accent">Visit <ArrowIcon /></a>{bonusClicks > 0 ? <span className="inline-flex items-baseline gap-1 tabular" aria-label={`${clicks.toLocaleString()} clicks, including ${bonusClicks.toLocaleString()} bonus`}><b className="text-lg font-semibold leading-none text-foreground">{clicks.toLocaleString()}</b><span>clicks ({bonusClicks.toLocaleString()} bonus)</span></span> : <span className="inline-flex items-baseline tabular" aria-label={`${clicks.toLocaleString()} clicks delivered out of ${paidClicks.toLocaleString()} paid`}><b className="text-lg font-semibold leading-none text-foreground">{clicks.toLocaleString()}</b><span>/{paidClicks.toLocaleString()} clicks</span></span>}</div>
    </article>
  );
}
